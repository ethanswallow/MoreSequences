...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  5 blue dots on a green circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(250, 150), 120)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  10 slices, thin (thickness=2) black lines, yellow circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 7
    circle.fill_color = 'yellow'
    pizza(window, circle, 10, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    circle.outline_thickness = 3
    polygon(window, circle, 3, 'black', 8)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        if k < number_of_segments - 1:
            ...
            num = k + 1 - number_of_segments
            line = rg.Line(points[k], points[num])
        line.color = color
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  300 yellow lines' + \
            ' on purple circle, hops = 61.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'purple'
    fancy_polygon(window, circle, 300, 61, 'yellow', 3)
    window.close_on_mouse_click()
